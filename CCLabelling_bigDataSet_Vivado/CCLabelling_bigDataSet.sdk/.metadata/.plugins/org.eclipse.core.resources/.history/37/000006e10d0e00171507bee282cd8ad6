...
bool verifyResult(u32 centroidCountSW, int *XSW, int *YSW, u32 centroidCountHW, int *XHW, int *YHW) {
	if(centroidCountSW == centroidCountHW) {
		for(int i = 0; i < (int)centroidCountHW; ++i) {
			if(XSW[i] == XHW[i] && YSW[i] == YHW[i])
				continue;
			return false;
		}
		return true;
